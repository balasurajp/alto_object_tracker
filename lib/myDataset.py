...
#import sys
#sys.path.append('../')
from Utils import center2corner, Center, center2corner, Corner
...
class myDataset(Dataset):
    def __init__(self, cfg):
        ...
        self.size = (self.search_size - self.template_size) // cfg.STRIDE + 1   # from cross-correlation
        ...
        if cfg.DATASET_NAME == 'INV':
            self.anno = cfg.INV_IMDB
            self.num_use = cfg.TRAIN_PAIRS
            self.root = cfg.INV_DATA
        elif cfg.DATASET_NAME == 'GOT10K':
            self.anno = cfg.GOT10K_IMDB
            self.num_use = cfg.TRAIN_PAIRS
            self.root = cfg.GOT10K_DATA
        else:
            raise ValueError('Unsupported training dataset')

        self.labels = json.load(open(self.anno, 'r'))
        self.videos = list(self.labels.keys())
        self.num = len(self.videos)   # video number
        self.frame_range = cfg.PAIR_RANGE
